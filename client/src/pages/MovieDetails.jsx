...
const MovieDetails = () => {
  ...
  const { axios, shows, image_base_url, user, favoriteMovies, navigate, getToken, fetchFavoriteMovies } = useAppContext();
    ...
    try {
      ...
      if (data.success) {
        if (data.movie) {
          ...
          toast.error("Invalid movie page");
        }
      } else {
      ...
      <div className='flex flex-col md:flex-row gap-8 max-w-6xl mx-auto'>
        ...
        <div className='relative flex flex-col gap-3'>
          ...
          <div className='flex items-center flex-wrap gap-4 mt-4'>
            <a href={`https://www.youtube.com/results?search_query=${show.movie.title + " movie trailer"}`} target="_blank" rel="noopener noreferrer">
            ...
            <button className='bg-gray-700 p-2.5 rounded-full transition cursor-pointer active:scale-95'>
              <Heart className={`w-5 h-5 ${favoriteMovies.find(movie => movie._id === id) ? "fill-primary text-primary" : ""}`} onClick={handleFavorite} />
            </button>
          </div>
        </div>
      </div>
        ...
        <div className='flex items-center gap-4 w-max px-4'>
          {show.movie.casts.slice(0, 12).map((cast, index) => (
            <div key={index} className='flex flex-col items-center text-center'>
              <img src={image_base_url + cast.profile_path} alt={cast.name} className='rounded-full h-20 md:h-20 aspect-square object-cover pointer-events-none' />
