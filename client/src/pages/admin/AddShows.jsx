...
const AddShows = () => {
  ...
  const handleSubmit = async () => {
    try {
      ...
      console.error(error)
      toast.error("And error occured. Please try again.");
    }
    setAddingShow(false);
  }
    ...
    <>
        ...
        <div className="group flex flex-wrap gap-4 mt-4 w-max">
          {nowPlayingMovies.map((movie) => (
            <div key={movie.id} onClick={() => setSelectedMovie(movie.id)} className={`relative max-w-40 cursor-pointer group-hover:not-hover:opacity-40 hover:-translate-y-1 transition duration-300 `}>
              ...
              <div className="absolute top-2 right-2 flex items-center justify-center h-6 w-6 rounded border">
              </div>
      ...
      <div className="mt-8">
        ...
        <div className="inline-flex items-center gap-2 border border-gray-600 px-3 py-2 rounded-md">
          ...
          <input id='show-price' min={0} type="number" value={showPrice} onChange={(e) => setShowPrice(e.target.value)} placeholder="Enter show price" className="outline-none" />
        </div>
      </div>

      {/* Date & Time Selection */}
      <div className="mt-6">
        <label htmlFor='select-date-time' className="block text-sm font-medium mb-2">Select Date and Time</label>
        <div className="inline-flex gap-5 border border-gray-600 p-1 pl-3 rounded-lg">
          <input id='select-date-time' type="datetime-local" value={dateTimeInput} onChange={(e) => setDateTimeInput(e.target.value)} className="outline-none rounded-md" />
          <button onClick={handleDateTimeAdd} className="bg-primary/80 text-white px-3 py-2 text-sm rounded-1g hover:bg-primary cursor-pointer">Add Time</button>
        </div>
      </div>
